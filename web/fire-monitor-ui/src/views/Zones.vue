...
<template>
    <div class="space-y-6 pb-20 pt-6">
        ...
        <Transition name="fade" mode="out-in">
            ...
            <!-- Empty State -->
            <div v-else-if="zones.length === 0" key="empty"
                ...
                <TransitionGroup name="list" tag="div" class="space-y-4 relative">
                    <div v-for="(zone, index) in zones" :key="zone.id"
                            ...
                            <div>
                                <div class="flex items-center gap-3">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" v-model="zone.enabled" class="sr-only peer"
                                            @change="toggleZone(zone)">
                                        <div
                                            class="w-8 h-4 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-emerald-500">
                                        </div>
                                    </label>
                                    <Transition name="pop" mode="out-in">
                                        <span :key="zone.enabled ? 'active' : 'disabled'"
                                            class="text-[9px] font-bold uppercase tracking-wider"
                                            :class="zone.enabled ? 'text-emerald-500' : 'text-text-muted'">
                                            {{ zone.enabled ? 'ON' : 'OFF' }}
                                        </span>
                                    </Transition>
                                </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button @click="openEditZone(zone)"
                                ...
                                style="background: rgba(245, 158, 11, 0.1);" title="编辑灶台">
                            ...
                            <button @click="deleteZone(zone.id)"
                                ...
                                style="background: rgba(239, 68, 68, 0.1);" title="删除">
        ...
        <Modal title="添加灶台" :is-open="showAddModal" @close="showAddModal = false">
            <div class="space-y-4">
                <div class="space-y-1">
                    ...
                    <input :value="nextZoneName" type="text" readonly
                        class="w-full rounded-xl px-4 py-3 border outline-none transition-all text-text-primary cursor-not-allowed opacity-70"
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        <select v-model="addForm.camera_id"
                            ...
                            style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                            <option v-for="cam in cameras" :key="cam.id" :value="cam.id" class="theme-select-option">{{
                                cam.name }}</option>
                        </select>
                        <!-- Chevron Icon -->
                ...
                <div class="space-y-1">
                    ...
                    <input v-model.number="addForm.fire_current_threshold" type="number" min="0" step="0.01"
                        placeholder="如: 1.45"
                        class="w-full rounded-xl px-4 py-3 border outline-none focus:border-primary/50 transition-all text-text-primary"
                        style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                    <p class="text-xs text-text-muted mt-1">实时电流超过此值（安培）则判定为动火</p>
                </div>
                <!-- 温度传感器开关 -->
                <div class="flex items-center justify-between p-4 rounded-2xl"
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center"
                            :class="addForm.enable_temp_sensor ? 'bg-orange-500/20 text-orange-400' : 'bg-gray-500/20 text-gray-400'">
                            🌡️
                        </div>
                        <div>
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div class="w-12 h-6 rounded-full peer transition-colors duration-300"
                            :class="addForm.enable_temp_sensor ? 'bg-orange-500' : 'bg-gray-500'">
                            <div class="absolute top-[2px] w-5 h-5 bg-white rounded-full shadow-md transition-all duration-300"
                                :class="addForm.enable_temp_sensor ? 'left-[26px]' : 'left-[2px]'"></div>
                        </div>
                    </label>
                </div>
        ...
        <Modal title="编辑灶台" :is-open="showEditModal" @close="showEditModal = false">
            <div class="space-y-4">
                ...
                <div class="space-y-1">
                    <label class="text-xs text-text-muted">名称</label>
                    <input :value="editForm.name" type="text" readonly
                        class="w-full rounded-xl px-4 py-3 border outline-none transition-all text-text-primary cursor-not-allowed opacity-70"
                        style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="space-y-1">
                        <label class="text-xs text-text-muted">电流阈值 (A)</label>
                        <input v-model.number="editForm.fire_current_threshold" type="number" min="0" step="0.01"
                            placeholder="1.00"
                            class="w-full rounded-xl px-4 py-3 border outline-none focus:border-primary/50 transition-all text-text-primary"
                            style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                    </div>
                ...
                <div class="flex items-center justify-between p-4 rounded-2xl"
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center"
                            :class="editForm.enable_temp_sensor ? 'bg-orange-500/20 text-orange-400' : 'bg-gray-500/20 text-gray-400'">
                            🌡️
                        </div>
                        <div>
                            <div class="font-medium text-text-primary text-sm">启用温度传感器</div>
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div class="w-12 h-6 rounded-full peer transition-colors duration-300"
                            :class="editForm.enable_temp_sensor ? 'bg-orange-500' : 'bg-gray-500'">
                            <div class="absolute top-[2px] w-5 h-5 bg-white rounded-full shadow-md transition-all duration-300"
                                :class="editForm.enable_temp_sensor ? 'left-[26px]' : 'left-[2px]'"></div>
                        </div>
                    </label>
                </div>
                <p v-if="editForm.enable_temp_sensor && !currentEditZone?.temp_sensor_address"
                ...
                <div class="space-y-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex justify-end gap-2">
                            <button @click="undoRoiPoint"
                                class="text-xs px-3 py-1.5 rounded-lg text-text-primary hover:text-white hover:bg-primary/80 transition-all border border-primary/30"
                                style="background: var(--theme-bg-input);">撤销</button>
                            <button @click="clearRoi"
                                class="text-xs px-3 py-1.5 rounded-lg text-text-primary hover:text-white hover:bg-red-500/80 transition-all border border-red-500/30"
                                style="background: var(--theme-bg-input);">清除区域</button>
                        </div>
                    </div>
                </div>

                <button @click="saveEditZone"
                    class="w-full py-3 bg-primary rounded-xl font-bold mt-4 hover:bg-primary-dark shadow-lg shadow-primary/20 transition-all active:scale-95 press-effect">
                    保存
                </button>
            </div>
