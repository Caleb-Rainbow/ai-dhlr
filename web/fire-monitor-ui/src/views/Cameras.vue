...
const openAddModal = async () => {
  ...
  form.value = { type: 'usb', device: undefined, width: 640, height: 480, fps: 30 };
...
<template>
  <div class="space-y-6 pb-20 pt-6">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-text-primary to-text-secondary">摄像头管理</h2>
    </div>
    ...
    <Transition name="fade" mode="out-in">
      ...
      <!-- Empty State -->
      <div v-else-if="cameras.length === 0" key="empty" class="flex flex-col items-center justify-center p-12 text-gray-500 bg-bg-card rounded-2xl border border-white/5">
        ...
        <TransitionGroup name="list" tag="div" class="space-y-4 relative">
          ...
          >
            ...
            <div class="flex gap-2 shrink-0">
               <button @click="openPreview(cam)" class="p-2.5 rounded-xl text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 transition-all border border-blue-500/30 press-effect" style="background: rgba(59, 130, 246, 0.1);">
                  <Eye class="w-4 h-4" />
               </button>
               <button @click="openEditModal(cam)" class="p-2.5 rounded-xl text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 transition-all border border-amber-500/30 press-effect" style="background: rgba(245, 158, 11, 0.1);">
                  <Settings class="w-4 h-4" />
               </button>
               <button @click="deleteCamera(cam.id)" class="p-2.5 rounded-xl text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all border border-red-500/30 press-effect" style="background: rgba(239, 68, 68, 0.1);">
    ...
    <Modal :title="isEditing ? '编辑摄像头' : '添加摄像头'" :is-open="showModal" @close="showModal = false">
        <div class="space-y-4">
            ...
            <div class="space-y-1">
                ...
                <div class="relative">
                    <select v-model="form.type" class="w-full rounded-xl px-4 py-3 border outline-none focus:border-primary/50 transition-all appearance-none cursor-pointer text-text-primary" style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
            ...
            <Transition name="slide-fade" mode="out-in">
              <div v-if="form.type === 'usb'" key="usb" class="space-y-1">
                  ...
                  <div v-else class="relative">
                      <select v-model="form.device" class="w-full rounded-xl px-4 py-3 border outline-none focus:border-primary/50 transition-all appearance-none cursor-pointer text-text-primary" style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                           <option v-for="dev in usbDevices" :key="dev.index" :value="dev.index" class="theme-select-option">{{ dev.name }}</option>
                      </select>
                      <div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none opacity-50">▼</div>
                  </div>
              </div>
              ...
              <div v-else-if="form.type === 'rtsp'" key="rtsp" class="space-y-2">
                  <div class="space-y-1">
                      ...
                      <input v-model="form.rtsp_url" type="text" placeholder="rtsp://..." class="w-full rounded-xl px-4 py-3 border outline-none focus:border-primary/50 transition-all text-text-primary" style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                  </div>
                  <div class="grid grid-cols-2 gap-2">
                      <div class="space-y-1">
                          <label class="text-xs text-text-muted">用户名</label>
                          <input v-model="form.username" type="text" class="w-full rounded-xl px-4 py-3 border outline-none focus:border-primary/50 transition-all text-text-primary" style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                      </div>
                       <div class="space-y-1">
                          <label class="text-xs text-text-muted">密码</label>
                          <input v-model="form.password" type="password" class="w-full rounded-xl px-4 py-3 border outline-none focus:border-primary/50 transition-all text-text-primary" style="background: var(--theme-bg-input); border-color: var(--theme-border-input);">
                      </div>
                  </div>
              </div>
            </Transition>

            <button @click="submitForm" class="w-full py-3 bg-primary rounded-xl font-bold mt-4 hover:bg-primary-dark shadow-lg shadow-primary/20 transition-all active:scale-95 press-effect">
                {{ isEditing ? '保存' : '添加' }}
            </button>
        </div>
    </Modal>
