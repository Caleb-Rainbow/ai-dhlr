        ...
        <TransitionGroup name="card">
          <div v-for="(zone, index) in enabledZones" :key="zone.id"
            ...
            <Transition name="fade">
              <div v-if="['alarm', 'cutoff'].includes(zone.state)"
                class="absolute inset-0 bg-red-500/10 animate-pulse z-0 pointer-events-none"></div>
            </Transition>
            ...
            <div class="relative z-10">
              ...
              <div class="mb-4 min-h-[2.5rem] flex items-center">
                <Transition name="slide-fade" mode="out-in">
                  <div v-if="['active_no_person', 'warning', 'alarm'].includes(zone.state)"
                    :key="zone.state + '-countdown'" class="flex items-baseline gap-1">
                    <span class="text-3xl font-bold font-mono tracking-tighter transition-colors duration-300"
                      ...
                      {{ Math.ceil(zone.state === 'active_no_person' ? zone.warning_remaining : (zone.state ===
                        'warning' ? zone.alarm_remaining : zone.cutoff_remaining)) }}
                    </span>
                    <span class="text-[10px] text-text-muted font-bold uppercase tracking-wider">秒剩余</span>
                  </div>
                  <div v-else-if="zone.state === 'cutoff'" :key="'cutoff'"
                    class="text-red-500 font-bold text-sm flex items-center gap-2">
      ...
      <div v-if="performance" key="perf"
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="col-span-2 grid grid-cols-3 gap-2">
            ...
            <div
              class="rounded-xl p-2 flex flex-col items-center justify-center overflow-hidden transition-all duration-300 hover-lift"
              style="background: var(--theme-bg-input);">
              <span class="text-[8px] text-text-muted uppercase mb-1">引擎</span>
              <span class="text-xs font-bold text-text-primary truncate w-full text-center px-1"
                :title="performance.engine">{{ performance.engine }}</span>
            </div>
          </div>
        </div>
      </div>
