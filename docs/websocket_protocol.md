...
#### query_lora_config — 广播查询 LoRa 配置

通过广播命令（`FF AA FF FF 03 00 3x 00 01`）向 LoRa 模块查询编号和信道，等待设备实际响应后返回。与 `get_lora_config` 的区别：`get_lora_config` 立即返回缓存的最近值，不与设备交互。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| success | boolean | 是否收到设备响应，任一查询命令收到响应即为 `true` |
| id | number | LoRa 编号（设备无响应时返回缓存的旧值，不覆盖） |
| channel | number | LoRa 信道（设备无响应时返回缓存的旧值，不覆盖） |
| message | string | 失败原因，仅失败时返回（如 `"串口未连接"`、`"查询超时"`） |

> **注意：** 设备无响应（LoRa 模块未接、链路不通）属于正常业务结果，以 `success: false` 返回而非错误响应。服务端最多等待约 8 秒（两条查询命令各 3 秒响应窗口），客户端请求超时建议不低于 15 秒。
...
开启后串口收发数据以 16 进制格式打印到日志：`[TX] FF AA FF FF 03 00 30 00 01 91 DB`（发送，广播查询 LoRa 编号）、`[RX] 01 03 02 00 05 ...`（接收）。
